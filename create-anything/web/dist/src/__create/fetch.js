...
export const fetchWithHeaders = async function fetchWithHeaders(...args) {
    ...
    try {
        const result = await originalFetch(`${isBackend() ? (process.env.NEXT_PUBLIC_CREATE_BASE_URL ?? 'https://www.create.xyz') : ''}${input}`, {
            ...init,
            headers: finalHeaders,
        });
        if (!result.ok) {
            postToParent('error', `Failed to load resource: the server responded with a status of ${result.status} (${result.statusText ?? ''})`, {
                url,
